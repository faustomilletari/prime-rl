...
def train(config: TrainingConfig):
    ...
    while True:
        ...
        for micro_step, micro_batch in enumerate(micro_batches, start=1):
            ...
            # Compute loss
            loss, importance_sampling_ratio = grpo_loss(
                logits,
                input_ids,
                advantages,
                logprobs,
            ...
            importance_sampling_ratio = importance_sampling_ratio / num_micro_batches
            ...
            loss_metrics["loss/importance_sampling_ratio"] += importance_sampling_ratio.detach().clone()

            del loss, entropy, importance_sampling_ratio
        ...
        step_message = f"Training     | step {progress.step} | Time:{step_time:.2f}s | Loss: {loss_metrics['loss/loss']:.2f} | Entropy: {loss_metrics['loss/entropy']:.2f} | Mean Ratio: {loss_metrics['loss/importance_sampling_ratio']:.2f} | {throughput:.0f} tokens/s | MFU: {mfu:.1f}%"
